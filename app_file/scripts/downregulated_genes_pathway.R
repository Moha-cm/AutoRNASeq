...
c_down_ind <- (data$log2FoldChange < -log2FoldChange) & (data$padj < padj)
c_down_ind[is.na(c_down_ind)] = FALSE
c_down_genes <- data$Gene_name[c_down_ind]
c_down_ensembl <- data$ensembl_gene_id[c_down_ind]

#  MOLECULAR FUNCTION

c_down_entrez <- bitr( c_down_ensembl, fromType = "ENSEMBL",
                         toType = c("ENTREZID"),
                         OrgDb = org.Hs.eg.db)

c_down_entrez <- c_down_entrez[!is.na(c_down_entrez)]



M_tab <- enrichGO(gene = c_down_ensembl,
                OrgDb = org.Hs.eg.db,
                keyType = 'ENSEMBL',
                ont = "MF",
                pAdjustMethod = "BH",
...
C_tab <- enrichGO(gene = c_down_ensembl,
                 OrgDb = org.Hs.eg.db,
                 keyType = 'ENSEMBL',
                 ont = "CC",
                 pAdjustMethod = "BH",
...
b_tab <- enrichGO(gene = c_down_ensembl,
                  OrgDb = org.Hs.eg.db,
                  keyType = 'ENSEMBL',
                  ont = "BP",
                  pAdjustMethod = "BH",
...
p_tab <- enrichKEGG(gene = c_down_entrez,
                   organism = 'hsa',
                   pvalueCutoff = 0.05)
  if(!is.null(p_tab)) {{
...
r_tab <- enrichPathway(gene = c_down_entrez, 
                       pvalueCutoff = 0.05, 
                       readable = TRUE)
  if(!is.null(r_tab)) {{
...
w_tab <- enrichWP(c_down_entrez, organism = "Homo sapiens")
  if(!is.null(w_tab)) {{
...
d_tab <- enrichDO(gene = c_down_entrez,
                  ont = "DO",
                  pvalueCutoff = 0.05,
                  pAdjustMethod = "BH",
                  qvalueCutoff = 0.05,
...
# nc_tab <- enrichNCG(gene = c_down_entrez,
#                    pvalueCutoff = 0.05,
#                    pAdjustMethod = "BH",
#                    qvalueCutoff = 0.05,
#                    readable = TRUE)
...
# GN_tab <- enrichDGN(gene = c_down_entrez,
#                    pvalueCutoff = 0.05,
#                    pAdjustMethod = "BH",
#                    qvalueCutoff = 0.05,
#                    readable = TRUE)
